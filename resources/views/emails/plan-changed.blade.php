...
<head>
    ...
    <title>Tu plan ha cambiado</title>
    <style>
        ...
        .badge { display: inline-block; background: #eff6ff; color: #1d4ed8; border: 1px solid #bfdbfe; border-radius: 6px; font-size: 12px; font-weight: 600; padding: 3px 10px; margin-bottom: 16px; }
        ...
        <div class="card">
            ...
            <div class="card-body">
                @php
                    ...
                    $newPlanLabel     = $planLabels[$subscription->plan_code] ?? ucfirst($subscription->plan_code);
                    $oldPlanLabel     = $planLabels[$oldPlanCode] ?? ucfirst($oldPlanCode);
                    $newIntervalLabel = $subscription->billing_interval === 'annual' ? 'Anual' : 'Mensual';
                    $oldIntervalLabel = $oldBillingInterval === 'annual' ? 'Anual' : 'Mensual';
                @endphp

                <div class="badge">Plan actualizado</div>
                <h1>Tu plan ha cambiado</h1>
                <p>
                    Hola <strong>{{ $user->name }}</strong>,
                    tu plan en NeuroCarta.ai® ha sido actualizado correctamente.
                </p>
                ...
                <div style="border: 1px solid #e5e7eb; border-radius: 10px; overflow: hidden; margin: 20px 0;">
                    <div class="detail-row">
                        ...
                        <span class="detail-value" style="color:#6b7280;">{{ $oldPlanLabel }} · {{ $oldIntervalLabel }}</span>
                    </div>
                    <div class="detail-row">
                        <span class="detail-label">Plan nuevo</span>
                        <span class="detail-value">{{ $newPlanLabel }} · {{ $newIntervalLabel }}</span>
                    </div>
