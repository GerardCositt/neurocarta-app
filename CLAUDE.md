...
- **Auditoría de preproducción completada (2026-05-29)**: Revisión exhaustiva de seguridad, Stripe, emails, multitenancy, variables de entorno, nginx, Dockerfile y deploy. Hallazgos: 2 CRÍTICO (API sin `isActive()`, Pro limit incorrecto), 5 ALTO (nginx headers, TrustProxies `*`, deploy como root, migrate en entrypoint, sin Sentry), varios MEDIO/BAJO. Todos los CRÍTICO y los ALTO seguros aplicados en commit `f97600a`. Pendientes confirmación: TrustProxies CIDR, migrate en entrypoint, usuario deploy, Sentry.
- **`AuthenticateApiKey` verificaba plan pero no actividad de la suscripción (2026-05-29)**: El middleware comprobaba `plan_code in ['pro', 'premium', 'trial']` pero no `$sub->isActive()`. Un restaurante con suscripción expirada podía seguir usando la API indefinidamente. Fix: añadir `! $sub || ! $sub->isActive()` a la condición de rechazo. Commit: `f97600a`.
- **Plan Pro límite de restaurantes corregido 1→2 (2026-05-29)**: `PlanEntitlementService::LIMITS` tenía `restaurants => 1` para Pro. La landing, documentación y tabla de precios dicen 2. Corrección en commit `f97600a`. El mensaje de error de límite también se corrigió para ser gramaticalmente correcto con cualquier valor (`incluye hasta {$limit} restaurantes`).
- **Headers de seguridad HTTP añadidos a nginx (2026-05-29)**: `docker/nginx.conf` ahora envía `X-Frame-Options: SAMEORIGIN`, `X-Content-Type-Options: nosniff` y `Referrer-Policy: strict-origin-when-cross-origin` en todas las respuestas HTTPS. Commit: `f97600a`. **Nota operacional**: `nginx -s reload` no es suficiente tras `git pull` — el bind mount de Docker usa el inode del archivo original y nginx recarga desde él (inode viejo, contenido antiguo). Solución: `docker compose -f docker-compose.prod.yml up -d --force-recreate nginx` para que el contenedor monte el inode actual.
- **`TrustProxies = '*'` — pendiente corrección (2026-05-29)**: Actualmente confía en todos los proxies, lo que permite falsificar `X-Forwarded-For` y anula el rate limiting por IP. Fix: sustituir por el CIDR de la red Docker interna. Para obtenerlo: `docker network inspect $(docker inspect --format='{{range $k, $v := .NetworkSettings.Networks}}{{$k}}{{end}}' neurocarta-nginx-1) | grep Subnet` en el servidor. Una vez obtenido, cambiar `$proxies = '*'` por el CIDR (ej. `'172.18.0.0/16'`).
...
## Estado Git (2026-05-29, último commit f97600a en main)
...
- **Stripe live activo**: claves y price IDs configurados en `.env` producción. Pendiente: configurar Customer Portal en Dashboard (Switch plans + 6 price IDs) y **prueba de pago real completa** (bloqueante para cobrar a clientes).
...
- **Activación de suscripción en `checkout/success`, no en el webhook (2026-05-29)**: La activación de suscripción, envío del email de confirmación y acceso a la carta pública ocurren ahora en `CheckoutController::success()` al recibir el `session_id` que Stripe pasa en la `success_url`. El método `activateFromCheckoutSession()` llama a la API de Stripe, activa la suscripción en BD y envía `SubscriptionActivated` inmediatamente. El webhook (`handleCheckoutCompleted`) sigue activo pero es **idempotente**: si `stripe_subscription_id` ya está guardado, devuelve 200 sin hacer nada. Motivación: el webhook puede llegar tarde, fallar silenciosamente o tener un `STRIPE_WEBHOOK_SECRET` mal configurado — si solo dependemos del webhook, la suscripción queda `inactive` indefinidamente. La `success_url` siempre se ejecuta en el navegador del usuario justo después del pago. Cache fallback `checkout_just_paid_{account_id}` (15 min, CACHE_DRIVER=file) se mantiene por si la llamada a Stripe en el success falla puntualmente. Commits: `3e767c7`, `5056f63`.
- **`SetPasswordController::createStripeCheckoutUrl()` alineado con `CheckoutController::create()` (2026-05-29)**: Se eliminaron `automatic_tax`, `billing_address_collection`, `tax_id_collection` y `customer_update` que habían quedado en `SetPasswordController` tras el fix del commit `69f7b38`. Estos campos causaban errores silenciosos en la creación del checkout si el dashboard de Stripe no tiene automatic tax configurado. Ambos flujos de checkout usan ahora `payment_method_types: ['card']` y sin campos de impuestos. Commit: `3e767c7`.
- **Hardening preproducción (2026-05-29)**: API verifica `isActive()` antes de autorizar acceso; plan Pro corregido a 2 restaurantes; headers `X-Frame-Options`, `X-Content-Type-Options` y `Referrer-Policy` activos en nginx producción. Commit: `f97600a`. Pendiente: `TrustProxies` (necesita CIDR Docker), usuario deploy dedicado, Sentry.
...
- [x] Headers de seguridad HTTP en nginx — `X-Frame-Options: SAMEORIGIN`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin` activos en producción. Commit `f97600a`.
- [x] API no acepta suscripciones expiradas — `AuthenticateApiKey` verifica `isActive()` además del `plan_code`. Commit `f97600a`.
- [ ] `TrustProxies = '*'` — pendiente sustituir por CIDR Docker. Obtener con: `docker network inspect ... | grep Subnet`.
